...
public class CrazyCircus<E extends Drawable> {
    ...
    private void connectControllers() {
        ...
        manager.addPropertyChangeListener(PodiumManager.PROP_FINISHED,
			...
			public void propertyChange(PropertyChangeEvent evt) {
				...
				enableButtons(!b);
				if (b) {
					output.append("\ngagné en " + manager.getShotsNb()
							+ " coups et " + manager.getTimeDelta() + "s.\n");
				}
				reinitPodiums();
			}
        });
        
        manager.addPropertyChangeListener(PodiumManager.PROP_LAST_ORDER,
        		new PropertyChangeListener() {
			@Override
			public void propertyChange(PropertyChangeEvent evt) {
        ...
        manager.addVetoableChangeListener(PodiumManager.PROP_LAST_ORDER,
        		new VetoableChangeListener() {
			@Override
			public void vetoableChange(PropertyChangeEvent evt)
					throws PropertyVetoException {
				if (!soAllower.isSelected() && evt.getNewValue() == Order.SO) {
					throw new PropertyVetoException("so", evt);
				}
			}
        });
    }
    ...
    private void enableButtons(boolean b) {
    	for (Order o : Order.values()) {
    		commandButtons.get(o).setEnabled(b);
    	}
    }
